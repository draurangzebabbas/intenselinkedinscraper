...
-- Create global_linkedin_profiles table (stores all scraped data, no image storage paths)
...
-- Create RLS policies for global_linkedin_profiles table (read-only for authenticated users)
...
-- Policies for system-level inserts/updates to global_linkedin_profiles (e.g., from functions or admin)
-- These are typically handled by service roles or specific functions, not direct user RLS.
-- For now, we'll assume direct inserts/updates are not allowed by regular authenticated users.
-- If you need to allow specific functions to write, you'd grant permissions to the function's invoker.
